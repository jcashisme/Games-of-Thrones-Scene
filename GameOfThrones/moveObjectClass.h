...
class ScreenBufferClass
{
private:

public:

    CONSOLE_CURSOR_INFO crsInfo = {1, 0}; //dwSize, bVisible

    COORD area;
    COORD coord = {0, 0}; //maybe use

    ScreenBufferClass(HANDLE hStdOut, int scrnWidth, int scrnHeight)
    {
        area = {scrnWidth, scrnHeight};

        //set curser to invisible
        SetConsoleCursorInfo(hStdOut, &crsInfo);

        //set window size
        SMALL_RECT windowSize = {0, 0, area.X - 1, area.Y - 1}; //declare and initialize window size
        SetConsoleWindowInfo(hStdOut, TRUE, &windowSize); //change console window size

        //set screen buffer size
        COORD bufferSize = {area.X * 1.5, area.Y * 1.5}; //create a COORD to hold the buffer size; buffer wider than window
...
public:
    ...
    //initialize printFrames: object in foreground
